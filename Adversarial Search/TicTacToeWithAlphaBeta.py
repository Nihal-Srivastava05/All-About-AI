...
def minmax(ai_board, depth, isMax, alpha, beta):
    ...
    if(isMax):
        ...
        for x in range(3):
            for y in range(3):
                if(ai_board[x][y] == '_'):
                    ai_board[x][y] = player
                    best = max(best, minmax(ai_board, depth+1, not isMax, alpha, beta))
                    ai_board[x][y] = '_'
        ...
        for x in range(3):
            for y in range(3):
                if(ai_board[x][y] == '_'):
                    ...
                    best = min(best, minmax(ai_board, depth+1, not isMax, alpha, beta))
                    ai_board[x][y] = '_'
                    beta = min(best, beta)
                    if(alpha >= beta):
                        break
...
def findBestMove(ai_board):
    ...
    for x in range(3):
        for y in range(3):
            if(ai_board[x][y] == '_'):
                ai_board[x][y] = player
                moveVal = minmax(ai_board, 0, False, -math.inf, math.inf)
                ai_board[x][y] = '_'
                if(moveVal > bestVal):
                    bestVal = moveVal
                    bestMove = (x, y)
